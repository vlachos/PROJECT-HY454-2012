...
void Load_Terrain_Text(std::string stageTxtPath){
	unsigned int load_counter_x = 0, load_counter_y = 0;

	std::ifstream openfile(stageTxtPath);
	if (openfile.is_open()){
		openfile >> terrain_x >> terrain_y;

		terrain = new unsigned char *[terrain_y];
		for (unsigned int i=0; i<terrain_y; ++i){
			terrain[i] = new unsigned char [terrain_x];
			for (unsigned int j=0; j<terrain_x; ++j){
		...
		std::cout << "Can not open file: "+stageTxtPath;
		std::exit(1);
	}
}

void Draw_Terrain(std::string bitmapPath){
	...
	tile_bitmap = al_load_bitmap((char*) bitmapPath.c_str());
	/*display each tile based on the already readen stage1.txt*/
	for (unsigned int i=0; i<terrain_x; ++i){
		for (unsigned int j=0; j<terrain_y; ++j){
...
void run_terrain_test(){
	Load_Terrain_Text( AppendToPath(GetTerrainBitmapDataPath(), STAGE1_TXT) );
	Draw_Terrain( AppendToPath(GetTerrainBitmapDataPath(), TILE_BITMAP_16) );
}
