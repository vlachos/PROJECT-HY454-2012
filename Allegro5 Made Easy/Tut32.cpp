// C++ ALLEGRO 5 MADE EASY TUTORIAL 32 - FADING [CAMERA EFFECTS]
// CODINGMADEEASY
...
int main()
{
	...
	float x = 10, y = 10, moveSpeed = 5, cameraPosition[2], fade;
	int dir = DOWN;
	bool active;
	int playerNumber = 0;
	...
	while(!done)
	{
		...
		else if (events.type == ALLEGRO_EVENT_TIMER)
		{
			if(events.timer.source == timer)
			{
				...
				fade = _camera[playerNumber].fade;
				...
				_camera[playerNumber].fade = fade;
		...
		if(draw)
		{
			...
			for(int i = 0; i < 2; i++)
			{
				...
				al_draw_tinted_bitmap(fader, al_map_rgba(0, 0, 0, _camera[i].fade), 0, 0, NULL);
